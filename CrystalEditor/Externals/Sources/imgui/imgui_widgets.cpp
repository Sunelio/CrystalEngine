...
void ImGui::ArrowEx(const char* str_id, ImGuiDir dir)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
...
bool ImGui::SquareButton(const char* str_id)
{
    float sz = GetFrameHeight();
    return SquareButtonEx(str_id, ImVec2(sz, sz), ImGuiButtonFlags_None);
}
...
bool ImGui::StraightLinesButton(const char* str_id)
{
    float sz = GetFrameHeight();
    return StraightLinesButtonEx(str_id, ImVec2(sz, sz), ImGuiButtonFlags_None);
}

bool ImGui::StraightLinesButtonEx(const char* str_id, ImVec2 size, ImGuiButtonFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, bg_col, true, g.Style.FrameRounding);
    RenderStraightLines(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), text_col);
...
void ImGui::ImageWithID(const char* str_id, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
bool ImGui::TreeNodeIconEx(const char* label, const ImTextureID& textureId, const ImVec2& textureSize, ImGuiTreeNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    return TreeNodeIconBehavior(window->GetID(label), textureId, textureSize, flags, label, NULL);
}
...
bool ImGui::TreeNodeIconBehavior(ImGuiID id, const ImTextureID& textureId, const ImVec2& textureSize, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (frame_bb.Max.y - frame_bb.Min.y < textureSize.y + style.FramePadding.y * 2)
        frame_bb.Max.y = textureSize.y + style.FramePadding.y * 2 + frame_bb.Min.y;
    ...
    ImGui::SameLine();
    ImGui::SetCursorPos({ImGui::GetCursorPos().x - 88, ImGui::GetCursorPos().y+3});
    ImRect bb(window->DC.CursorPos, window->DC.CursorPos + textureSize);
    ...
    if (display_frame)
    {
        ...
        {
            RenderBullet(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, (frame_bb.Max.y + frame_bb.Min.y)*0.5f-8), text_col);
            RenderImage(window->DrawList, textureId, bb.Min + ImVec2(4, 0), bb.Max + ImVec2(4, 0), { 0, 1 }, { 1, 0 }, GetColorU32(ImVec4(1, 1, 1, 1)));
        }
        else if (!is_leaf)
        {
            const auto posText = ImVec2(text_pos.x - text_offset_x + padding.x, (frame_bb.Max.y + frame_bb.Min.y)*0.5f-8);
            auto posStartImage = bb.Min + ImVec2(4, 0);
            auto posEndImage = bb.Max + ImVec2(4, 0);

            const auto imageUndernderflow = posText.x - posStartImage.x + 20;
            posStartImage.x += imageUndernderflow;
            posEndImage.x += imageUndernderflow;
            
            RenderArrow(window->DrawList, posText, text_col, is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.0f);
            RenderImage(window->DrawList, textureId, posStartImage, posEndImage, { 0, 1 }, { 1, 0 }, GetColorU32(ImVec4(1, 1, 1, 1)));
        }
        ...
        RenderTextClipped(ImVec2(text_pos.x + textureSize.x, (frame_bb.Max.y + frame_bb.Min.y)*0.5f-8), frame_bb.Max, label, label_end, &label_size);
    }
    else
    {
        ...
        RenderNavHighlight(frame_bb, id, nav_highlight_flags);
        if (flags & ImGuiTreeNodeFlags_Bullet)
            RenderImage(window->DrawList, textureId, bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), { 0, 1 }, { 1, 0 }, GetColorU32(ImVec4(1, 1, 1, 1)));
        else if (!is_leaf)
            RenderImage(window->DrawList, textureId, bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), { 0, 1 }, { 1, 0 }, GetColorU32(ImVec4(1, 1, 1, 1)));
        if (g.LogEnabled)
            LogSetNextTextDecoration(">", NULL);
        
        RenderText(text_pos + ImVec2(10, 0), label, label_end, false);
    }
    frame_bb.Max.y -= 20;
    ImGui::SetCursorPos(frame_bb.Max);
    ImGui::Dummy({0, 5});
...
bool ImGui::CollapsingHeaderWithIcon(const char* label, const ImTextureID& textureId, const ImVec2& textureSize, ImGuiTreeNodeFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    
    return TreeNodeIconBehavior(window->GetID(label), textureId, textureSize, flags | ImGuiTreeNodeFlags_CollapsingHeader, label);
}
