...
void main()
{
    ...
    gl_Position    = mvpMatrix * aInstanceMat * vec4(animatedPos.xyz, 1.0);
	fragPos        = (modelMat * aInstanceMat * animatedPos).xyz;
	fragTexCoord   = aTexCoord;
	fragNormal     = normalize(mat3(modelMat * aInstanceMat) * animatedNormal   );
	vec3 tangent   = normalize(mat3(modelMat * aInstanceMat) * animatedTangent  );
	vec3 bitangent = normalize(mat3(modelMat * aInstanceMat) * animatedBitangent);
