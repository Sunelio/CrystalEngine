...
template <> Maths::Vector3 Maths::Matrix<3, 3>::operator*(const Vector3& val) const
{
    return Vector3
    (
...
template <> Maths::Vector4 Maths::Matrix<4, 4>::operator*(const Vector4& val) const
{
    return Vector4
    (
...
template <> Maths::Mat3 Maths::Matrix<3, 3>::Inv3() const
{
    ...
    return Mat3(val.m[0][0], val.m[0][1], val.m[0][2],
...
template <> Maths::Quaternion Maths::Matrix<3, 3>::ToQuaternion() const
{
    ...
    for (int i = 0; i < 3; i++)
    {
        Vector3 xScale     = { mat[0][i], mat[1][i], mat[2][i] };
        const float length = xScale.GetLength();
        mat[0][i] = mat[0][i] / length;
        mat[1][i] = mat[1][i] / length;
        mat[2][i] = mat[2][i] / length;
    }

    // Get rotation from the matrix.
    const float w = -1 * sqrtf(1 + mat[0][0] + mat[1][1] + mat[2][2]) * 0.5f;
    return Quaternion(w,
