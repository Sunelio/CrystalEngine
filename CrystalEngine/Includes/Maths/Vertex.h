    ...
    template<> struct hash<Maths::TangentVertex>
    {
        size_t operator()(Maths::TangentVertex const& vertex) const noexcept
        {
            return ((hash<Maths::Vector3>()(vertex.pos) ^
                    (hash<Maths::Vector2>()(vertex.uv) << 1)) >> 1) ^
                    (hash<Maths::Vector3>()(vertex.normal) << 1) ^
                    (hash<Maths::Vector3>()(vertex.tangent) << 2) ^
                    (hash<Maths::Vector3>()(vertex.bitangent) << 3);
        }
    };

    template<> struct hash<Maths::VertexIndices>
    {
        size_t operator()(Maths::VertexIndices const& vertex) const noexcept
        {
            return ((hash<uint32_t>()(vertex.pos) ^
                    (hash<uint32_t>()(vertex.uv) << 1)) >> 1) ^
                    (hash<uint32_t>()(vertex.normal) << 1);
        }
    };
}